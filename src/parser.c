...
void parser_parse_whitespace(Parser* parser, AST_Node* parent) {
  ...
  if (lexer_is_token_whitespace(first_token)) {
    return;
  }
  
  // TODO(fz): This could be a table
  AST_Node_Type node_type = (first_token.type == Token_Space) ? AST_Node_Space : (first_token.type == Token_Tab)   ? AST_Node_Tab   : AST_Node_New_Line;
...
void parser_advance(Parser* parser, AST_Node* parent) {
  parser_parse_whitespace(parser, parent);
  parser_parse_comment_line(parser, parent);
  parser_parse_comment_block(parser, parent);
...
b32 parser_parse_declaration(Parser* parser) {
  ...
  // TODO(fz): Add other declaration types here
  // else if (parser_parse_struct_declaration(parser)) { result = true; }
  // else if (parser_parse_union_declaration(parser)) { result = true; }
  // else if (parser_parse_enum_declaration(parser)) { result = true; }
...
void ast_node_add_child(Arena* arena, AST_Node* parent, AST_Node* child) {
  ...
  if (parent->children_count > 0 && (parent->children_count & (parent->children_count - 1)) == 0) {
	...
	// TODO(Fz): We can just do a MemoryCopy here.
